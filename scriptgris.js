...
const STATUS_OPTIONS = ['Cerrada', 'Escalada'];
const PDF_PALETTE = {
  taupe: [74, 67, 66],
  olive: [168, 160, 124],
  cream: [226, 218, 205],
...
function buildMetricsCharts(snapshot) {
  ...
  const userLabels = usersRows.map(row => row[0]);
  const userTotals = usersRows.map(row => Number(row[1] || 0));
...
async function generarPDF(data) {
  ...
  const drawInfoChunks = (rows, headFillColor) => {
    ...
    for (let i = 0; i < rows.length; i += 3) {
      ...
      doc.autoTable({
        ...
        theme: 'grid',
        styles: { fontSize: 9, cellPadding: 4, valign: 'middle', halign: 'left', lineColor: PDF_PALETTE.line, textColor: PDF_PALETTE.text, font: 'times' },
        headStyles: { fillColor: headFillColor, textColor: [0, 0, 0], halign: 'center', fontSize: 9.2, fontStyle: 'bold', font: 'times' },
  ...
  if (complementaryRows.length) {
    ...
    drawInfoChunks(complementaryRows, [255, 255, 255]);
    startY += sectionSpacing;
  }
...
function getTicketMetricUser(ticket) {
  return resolveDisplayName(ticket.tecnico || ticket.raw?.USUARIO || ticket.raw?.usuario || 'Sin técnico');
}
function populateMetricsFilters(tickets) {
  ...
  const excludedTechKeys = getAnalyticsExcludedTechnicianKeys();
  const users = [...new Set((tickets || []).map(getTicketMetricUser).filter(Boolean).filter(name => !excludedTechKeys.has(technicianKey(name))))].sort((a, b) => a.localeCompare(b, 'es'));
  ...
  statusSelect.innerHTML = '<option value="">Todos</option>' + [...new Set([...STATUS_OPTIONS, ...statuses])].map(status => `<option value="${escapeHtml(status)}">${escapeHtml(status)}</option>`).join('');
  if (currentUser && users.includes(currentUser)) userSelect.value = currentUser;
  if (currentStatus && [...new Set([...STATUS_OPTIONS, ...statuses])].includes(currentStatus)) statusSelect.value = currentStatus;
}
...
function buildMetricsProductivityRows(tickets) {
  const users = new Map();
  const excludedTechKeys = getAnalyticsExcludedTechnicianKeys();
  tickets.forEach(ticket => {
    const user = getTicketMetricUser(ticket) || 'Sin usuario';
    if (excludedTechKeys.has(technicianKey(user))) return;
...
async function renderMetricsDashboard() {
  ...
  const productivityRows = buildMetricsProductivityRows(filtered);
...
/* === AJUSTE FINAL: ESTATUS, MÉTRICAS, TIENDA Y PDF === */
try { if (Array.isArray(STATUS_OPTIONS)) STATUS_OPTIONS.splice(0, STATUS_OPTIONS.length, 'Cerrada', 'Escalada'); } catch(e) {}

function normalizeFieldKeyFinal(value) {
  return safeText(value).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]/g, '');
}
function getRecordValueFinal(record, names = []) {
  if (!record || typeof record !== 'object') return '';
  const keys = Object.keys(record);
  const targets = names.map(normalizeFieldKeyFinal);
  for (const target of targets) {
    const key = keys.find(k => normalizeFieldKeyFinal(k) === target);
    if (key && safeText(record[key])) return safeText(record[key]);
  }
  for (const target of targets) {
    const key = keys.find(k => {
      const kk = normalizeFieldKeyFinal(k);
      return kk.includes(target) || target.includes(kk);
    });
    if (key && safeText(record[key])) return safeText(record[key]);
  }
  return '';
}
function isShortSystemUserFinal(value) {
  const v = safeText(value).toLowerCase();
  if (!v) return true;
  if (v.includes('@') || v.includes('\\')) return true;
  return /^[a-z0-9._-]{3,28}$/.test(v) && !v.includes(' ');
}
function preferredUserNameFromRecordFinal(record, fallback = '') {
  const candidates = [
    getRecordValueFinal(record, ['FondoGeneral', 'Fondo General', 'Nombre Completo', 'NOMBRE COMPLETO', 'Nombre y Apellido', 'Nombre', 'NOMBRE', 'Tecnico', 'Técnico', 'TECNICO']),
    getRecordValueFinal(record, ['Usuario', 'USUARIO', 'user', 'USER']),
    fallback
  ].map(normalizeName).filter(Boolean);
  return candidates.find(n => n.split(/\s+/).length >= 2 && !isShortSystemUserFinal(n)) || candidates[0] || normalizeName(fallback) || 'Sin usuario';
}
function loginKeyFromRecordFinal(record) {
  return getRecordValueFinal(record, ['Usuario', 'USUARIO', 'user', 'USER', 'Correo', 'Email', 'Mail']);
}
function findUserRecordAnyFinal(value) {
  const input = safeText(value);
  if (!input || !Array.isArray(APP.usersAPI)) return null;
  const inputKey = normalizeUserKey(input);
  const inputNameKey = normalizeFieldKeyFinal(input);
  return APP.usersAPI.find(record => {
    const candidates = [
      loginKeyFromRecordFinal(record),
      getRecordValueFinal(record, ['FondoGeneral', 'Fondo General', 'Nombre Completo', 'Nombre', 'NOMBRE', 'Tecnico', 'Técnico', 'TECNICO']),
      getRecordValueFinal(record, ['Correo', 'Email', 'Mail'])
    ].filter(Boolean);
    return candidates.some(candidate => normalizeUserKey(candidate) === inputKey || normalizeFieldKeyFinal(candidate) === inputNameKey);
  }) || null;
}
function resolveDisplayName(userInput) {
  const rawOriginal = safeText(userInput);
  const raw = rawOriginal.toLowerCase().replace(/\s+/g, ' ').trim();
  const aliases = {
    'mmontiel ext': 'Mario Montiel Zapien',
    'mmontiel.ext': 'Mario Montiel Zapien',
    'mmontiel': 'Mario Montiel Zapien',
  ...
  if (aliases[raw]) return aliases[raw];
  const record = findUserRecordAnyFinal(rawOriginal);
  if (record) return preferredUserNameFromRecordFinal(record, rawOriginal);
  return normalizeName(rawOriginal) || 'Sin usuario';
}
function technicianKey(name) {
  return safeText(resolveDisplayName(name)).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]/g, '');
}
function normalizeStatusLabel(value) {
  const raw = safeText(value).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  if (!raw) return '';
  if (raw.includes('escal')) return 'Escalada';
  if (raw.includes('cerr') || raw.includes('resuelt') || raw.includes('solucion') || raw.includes('finaliz') || raw.includes('terminad') || raw.includes('atendid')) return 'Cerrada';
  return '';
}
function findDynamicValue(data, ids = [], labelsIncludes = []) {
  const fields = Array.isArray(data?.dynamicFields) ? data.dynamicFields : [];
  const labelKeys = labelsIncludes.map(normalizeFieldKeyFinal);
  const found = fields.find(field => {
    const id = safeText(field?.id);
    const label = normalizeFieldKeyFinal(field?.label || '');
    return ids.includes(id) || labelKeys.some(key => label.includes(key));
  });
  return found ? safeText(found.value) : '';
}
function getStatusDynamicFields() {
  const status = normalizeStatusLabel($('casoStatus')?.value);
  const escaladaArea = safeText($('casoEscaladaArea')?.value);
  const rows = [];
  if (status) rows.push({ id: 'casoStatusSolicitud', label: 'Status de la solicitud', value: status, required: false });
  if (status === 'Escalada' && escaladaArea) rows.push({ id: 'casoAreaEscalada', label: 'Área a la que se escaló', value: escaladaArea, required: true });
  return rows;
}
function getTicketStatus(data) {
  const raw = data?.raw || data || {};
  return normalizeStatusLabel(firstNonEmpty_(
    data?.status,
    findDynamicValue(data || {}, ['casoStatusSolicitud'], ['status de la solicitud', 'estatus de la solicitud', 'status solicitud']),
    getRecordValueFinal(raw, ['STATUSSOLICITUD', 'STATUS SOLICITUD', 'ESTATUSSOLICITUD', 'ESTATUS DE LA SOLICITUD', 'STATUS', 'ESTATUS', 'Status de la solicitud', 'Estatus'])
  ));
}
function getTicketEscaladaArea(data) {
  const raw = data?.raw || data || {};
  return safeText(firstNonEmpty_(
    data?.escaladaArea,
    findDynamicValue(data || {}, ['casoAreaEscalada'], ['área a la que se escaló', 'area a la que se escalo', 'area escalada']),
    getRecordValueFinal(raw, ['AREAESCALADA', 'AREA ESCALADA', 'ÁREA A LA QUE SE ESCALÓ', 'AREA A LA QUE SE ESCALO'])
  ));
}
function updateStatusFieldVisibility() {
  const group = $('escaladaAreaGroup');
  const input = $('casoEscaladaArea');
  if (!group || !input) return;
  const show = normalizeStatusLabel($('casoStatus')?.value) === 'Escalada';
  group.classList.toggle('hidden', !show);
  if (!show) input.value = '';
}
function getDynamicFieldsForSaveFinal(data) {
  const base = Array.isArray(data.dynamicFields) ? data.dynamicFields.filter(f => !['casoStatusSolicitud', 'casoAreaEscalada'].includes(f?.id)) : [];
  const status = normalizeStatusLabel(data.status);
  const escaladaArea = safeText(data.escaladaArea);
  if (status) base.push({ id: 'casoStatusSolicitud', label: 'Status de la solicitud', value: status, required: false });
  if (status === 'Escalada' && escaladaArea) base.push({ id: 'casoAreaEscalada', label: 'Área a la que se escaló', value: escaladaArea, required: true });
  return base;
}
function collectFormData() {
  const status = normalizeStatusLabel($('casoStatus')?.value);
  const data = {
    ...
    folio: sanitizeDigits($('casoFolio').value),
    fecha: normalizeDateOnly($('casoFecha').value),
    tecnico: resolveDisplayName(safeText($('casoTecnico').value) || APP.currentUser),
    marca: safeText($('casoMarca').value),
    tienda: safeText($('casoTienda').value),
    servicio: safeText($('casoServicio').value),
    status,
    escaladaArea: safeText($('casoEscaladaArea')?.value),
    dynamicFields: getDynamicFieldValues(),
    checklists: getAnsweredChecklistResponses(),
    evidencias: APP.images.map((item, index) => normalizeEvidenceItem(item, index) || { ...item }),
    updatedAt: Date.now(),
  ...
  data.dynamicFields = getDynamicFieldsForSaveFinal(data);
  return data;
}
function validateFormData(data) {
  const missing = [];
  ...
  (Array.isArray(data.dynamicFields) ? data.dynamicFields : []).forEach(field => {
    if (field.required && !safeText(field.value)) missing.push(field.label);
  });
  if (normalizeStatusLabel(data.status) === 'Escalada' && !safeText(data.escaladaArea)) missing.push('Área a la que se escaló');
  return [...new Set(missing)];
}
function buildApiPayload(data) {
  const dynamicFields = getDynamicFieldsForSaveFinal(data);
  const dataForFind = { ...data, dynamicFields };
  const evidenceMeta = buildFastEvidenceMeta(data);
  const status = normalizeStatusLabel(data.status);
  const escaladaArea = status === 'Escalada' ? safeText(data.escaladaArea) : '';
  const tienda = safeText(data.tienda);
  const tecnico = resolveDisplayName(data.tecnico || APP.currentUser);
  const loginUser = loginKeyFromRecordFinal(APP.currentLoginRecord || {}) || APP.currentLoginUserKey || tecnico;
  return {
    FOLIO: data.folio,
    FECHA: normalizeDateOnly(data.fecha),
    TECNICO: tecnico,
    ...
    SUCURSAL: tienda,
    TIENDA: tienda,
    SUCURSALTIENDA: tienda,
    NOMBRETIENDA: tienda,
    AREAUSUARIO: findDynamicValue(dataForFind, ['casoArea'], ['área']),
    TIPODEEQUIPO: findDynamicValue(dataForFind, ['casoEquipo'], ['equipo']),
    NUMEROSERIE: findDynamicValue(dataForFind, ['casoSerie'], ['serie']),
    DESCRIPCIONBREVE: findDynamicValue(dataForFind, ['casoDescripcion', 'casoProblemaRed'], ['descripción', 'descripcion', 'problema']),
    ERRORDETECTADO: findDynamicValue(dataForFind, ['casoTipoFalla'], ['falla']),
    DIAGNOSTICO: findDynamicValue(dataForFind, ['casoDiagnostico'], ['diagnóstico', 'diagnostico']),
    SOLUCION: findDynamicValue(dataForFind, ['casoComentario'], ['comentario', 'solucion', 'solución']),
    CHECKLIST: JSON.stringify(data.checklists || { checklist1: [], checklist2: [] }),
    CHECKLIST1: JSON.stringify((data.checklists && data.checklists.checklist1) || []),
    CHECKLIST2: JSON.stringify((data.checklists && data.checklists.checklist2) || []),
    CAMPOSDINAMICOS: JSON.stringify(dynamicFields),
    TIPOSERVICIO: data.servicio,
    STATUSSOLICITUD: status,
    ESTATUS: status,
    STATUS: status,
    AREAESCALADA: escaladaArea,
    EVIDENCIASCOUNT: evidenceMeta.count,
    EVIDENCIASNOMBRES: evidenceMeta.namesJoined,
    FOTOSDIAGNOSTICOCOUNT: evidenceMeta.count,
    FOTOSSOLUCIONCOUNT: 0,
...
function stripImagesForPersistence(data) {
  ...
  return {
    ...data,
    tecnico: resolveDisplayName(data.tecnico || APP.currentUser),
    tienda: safeText(data.tienda),
    status,
    escaladaArea: status === 'Escalada' ? safeText(data.escaladaArea) : '',
    dynamicFields: getDynamicFieldsForSaveFinal(data),
    evidencias: (Array.isArray(data.evidencias) ? data.evidencias : []).map((item, index) => {
      const normalized = normalizeEvidenceItem(item, index);
      return normalized ? { name: normalized.name, dataUrl: '', previewUrl: normalized.previewUrl, sourceUrl: normalized.sourceUrl, previewCandidates: normalized.previewCandidates || [] } : null;
    }).filter(Boolean),
    evidenciasCount: meta.count,
    evidenciasNombres: meta.namesJoined
  };
...
function normalizeRemoteTicket(record, index) {
  let parsedChecklist = { checklist1: [], checklist2: [] };
  try {
    const parsed = JSON.parse(getRecordValueFinal(record, ['CHECKLIST', 'checklist', 'CHECKLIST1']) || '');
    if (parsed && typeof parsed === 'object') parsedChecklist = { checklist1: parsed.checklist1 || [], checklist2: parsed.checklist2 || [] };
  } catch(e) {}
  let dynamicFields = [];
  try {
    const parsed = JSON.parse(getRecordValueFinal(record, ['CAMPOSDINAMICOS', 'CAMPOS DINAMICOS', 'camposDinamicos']) || '[]');
    if (Array.isArray(parsed)) dynamicFields = parsed;
  } catch(e) {}
  if (!dynamicFields.length) {
    const flatPairs = [
      ['casoArea', 'Área', getRecordValueFinal(record, ['AREAUSUARIO', 'AREA USUARIO', 'Área', 'Area'])],
      ['casoEquipo', 'Equipo', getRecordValueFinal(record, ['TIPODEEQUIPO', 'TIPO DE EQUIPO', 'Equipo'])],
      ['casoSerie', 'No. Serie', getRecordValueFinal(record, ['NUMEROSERIE', 'NUMERO SERIE', 'NUMERO DE SERIE', 'No Serie'])],
      ['casoDescripcion', 'Descripción del problema', getRecordValueFinal(record, ['DESCRIPCIONBREVE', 'DESCRIPCION BREVE', 'DESCRIPCION', 'Descripción'])],
      ['casoDiagnostico', 'Diagnóstico', getRecordValueFinal(record, ['DIAGNOSTICO', 'Diagnóstico'])],
      ['casoComentario', 'Comentario', getRecordValueFinal(record, ['SOLUCION', 'SOLUCIÓN', 'COMENTARIO', 'Comentario'])]
    ];
    dynamicFields = flatPairs.filter(([, , value]) => safeText(value)).map(([id, label, value]) => ({ id, label, value, required: false }));
  }
  const status = normalizeStatusLabel(firstNonEmpty_(
    findDynamicValue({ dynamicFields }, ['casoStatusSolicitud'], ['status de la solicitud', 'estatus de la solicitud']),
    getRecordValueFinal(record, ['STATUSSOLICITUD', 'STATUS SOLICITUD', 'ESTATUSSOLICITUD', 'ESTATUS DE LA SOLICITUD', 'STATUS', 'ESTATUS'])
  ));
  const escaladaArea = safeText(firstNonEmpty_(
    findDynamicValue({ dynamicFields }, ['casoAreaEscalada'], ['área a la que se escaló', 'area a la que se escalo', 'area escalada']),
    getRecordValueFinal(record, ['AREAESCALADA', 'AREA ESCALADA', 'AREA A LA QUE SE ESCALO', 'ÁREA A LA QUE SE ESCALÓ'])
  ));
  const tienda = getRecordValueFinal(record, ['SUCURSAL', 'TIENDA', 'SUCURSALTIENDA', 'NOMBRETIENDA', 'Tienda', 'Sucursal', 'Sucursal o tienda', 'Sitio o tienda', 'NOMBRE COMERCIAL']);
  return {
    id: `api:${index}`,
    source: 'api',
    folio: getRecordValueFinal(record, ['FOLIO', 'Folio']),
    fecha: normalizeDateOnly(getRecordValueFinal(record, ['FECHA', 'Fecha'])),
    tecnico: resolveDisplayName(getRecordValueFinal(record, ['TECNICO', 'Técnico', 'NOMBREUSUARIO', 'USUARIO', 'usuario'])),
    marca: getRecordValueFinal(record, ['MARCA', 'Marca']),
    tienda,
    servicio: getRecordValueFinal(record, ['TIPOSERVICIO', 'TIPO SERVICIO', 'Tipo de servicio', 'SERVICIO', 'Servicio']),
    status,
    escaladaArea,
    dynamicFields,
    checklists: parsedChecklist,
    evidencias: extractEvidenceFromRecord(record),
    updatedAt: Date.parse(getRecordValueFinal(record, ['timestamp', 'TIMESTAMP', 'FECHA']) || '') || Date.now(),
    createdAt: Date.parse(getRecordValueFinal(record, ['timestamp', 'TIMESTAMP', 'FECHA']) || '') || Date.now(),
    raw: record
  };
}
function mergeTickets(remoteTickets, localTickets) {
  ...
  localTickets.forEach(ticket => {
    ...
    const localTicket = {
      ...
      id: `local:${ticket.id}`,
      source: 'local',
      tecnico: resolveDisplayName(firstNonEmpty_(ticket.tecnico, remoteTicket?.tecnico, APP.currentUser)),
      tienda: safeText(firstNonEmpty_(ticket.tienda, remoteTicket?.tienda)),
      servicio: safeText(firstNonEmpty_(ticket.servicio, remoteTicket?.servicio)),
      marca: safeText(firstNonEmpty_(ticket.marca, remoteTicket?.marca)),
      status: normalizeStatusLabel(firstNonEmpty_(ticket.status, remoteTicket?.status)),
      escaladaArea: safeText(firstNonEmpty_(ticket.escaladaArea, remoteTicket?.escaladaArea))
    };
    localTicket.evidencias = (Array.isArray(ticket.evidencias) && ticket.evidencias.length) ? ticket.evidencias : (Array.isArray(remoteTicket?.evidencias) ? remoteTicket.evidencias : []);
    localTicket.dynamicFields = (Array.isArray(ticket.dynamicFields) && ticket.dynamicFields.length) ? ticket.dynamicFields : (Array.isArray(remoteTicket?.dynamicFields) ? remoteTicket.dynamicFields : []);
    localTicket.checklists = (ticket.checklists && ((ticket.checklists.checklist1 || []).length || (ticket.checklists.checklist2 || []).length)) ? ticket.checklists : (remoteTicket?.checklists || { checklist1: [], checklist2: [] });
    localTicket.raw = remoteTicket?.raw || ticket.raw || null;
    if (key === '|') map.set(`local:${ticket.id}`, localTicket);
    else map.set(key, localTicket);
  });
  return Array.from(map.values()).sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0));
}
async function editTicketByRef(ref) {
  ...
  $('casoTienda').value = ticket.tienda || '';
  $('casoServicio').value = ticket.servicio || '';
  if ($('casoStatus')) $('casoStatus').value = getTicketStatus(ticket) || '';
  updateStatusFieldVisibility();
  if ($('casoEscaladaArea')) $('casoEscaladaArea').value = getTicketEscaladaArea(ticket) || '';
  renderDynamicForm();
  (ticket.dynamicFields || []).forEach(field => { if ($(field.id)) $(field.id).value = field.value || ''; });
  if ($('casoStatus')) $('casoStatus').value = getTicketStatus(ticket) || '';
  updateStatusFieldVisibility();
...
function buildGeneralRows(data) {
  return [
    ['Folio', data.folio],
    ['Fecha', normalizeDateOnly(data.fecha)],
    ['Técnico', resolveDisplayName(data.tecnico)],
    ['Marca', data.marca],
    ['Tienda', data.tienda],
    ['Tipo de servicio', data.servicio],
    ['Status', getTicketStatus(data)]
...
function buildPdfComplementaryRows(data, legacy) {
  const map = getDynamicFieldMap(data);
  const service = safeText(data.servicio || legacy.TIPOSERVICIO);
  const skipIds = new Set(['casoArea', 'casoEquipo', 'casoSerie', 'casoDescripcion', 'casoProblemaRed', 'casoDiagnostico', 'casoComentario', 'casoSitio', 'casoTipoFalla', 'casoGerente', 'casoContacto', 'casoHorario', 'casoAfiliacion', 'casoReporteVerifone', 'casoGerenteTurno', 'casoTelefonoTurno', 'casoHorarioVisita', 'casoUsuarioReset', 'casoSistemaReset', 'casoTipoReset', 'casoStatusSolicitud', 'casoAreaEscalada']);
  const serviceSpecific = [];
  const push = (label, value) => { const clean = safeText(value); if (clean) serviceSpecific.push([label, clean]); };
  const status = getTicketStatus(data);
  if (status) push('Status de la solicitud', status);
  if (status === 'Escalada') push('Área a la que se escaló', getTicketEscaladaArea(data));
  if (service === 'SIC') {
    push('Dirección exacta de la tienda', map.casoDireccion);
    push('Referencias físicas de la tienda', map.casoReferencias);
    push('Cargador', map.casoCargador);
  }
  const remaining = (data.dynamicFields || []).filter(field => !skipIds.has(field.id)).filter(field => safeText(field.value)).map(field => [field.label, safeText(field.value)]);
  return [...serviceSpecific, ...remaining];
}
function drawPdfHeader(doc, pageNumber, totalPages) {
  const pageWidth = doc.internal.pageSize.getWidth();
...
function getAllMetricUsersFinal() {
  const excluded = getAnalyticsExcludedTechnicianKeys();
  const map = new Map();
  if (Array.isArray(APP.usersAPI)) {
    APP.usersAPI.forEach(record => {
      const name = preferredUserNameFromRecordFinal(record, loginKeyFromRecordFinal(record));
      if (!name || excluded.has(technicianKey(name))) return;
      map.set(technicianKey(name), name);
    });
  }
  (APP.mergedTickets || []).forEach(ticket => {
    const name = getTicketMetricUser(ticket);
    if (!name || excluded.has(technicianKey(name))) return;
    map.set(technicianKey(name), name);
  });
  return [...map.values()].sort((a, b) => a.localeCompare(b, 'es', { sensitivity: 'base' }));
}
function getTicketMetricStatus(ticket) { return normalizeStatusLabel(getTicketStatus(ticket)); }
function getTicketMetricUser(ticket) {
  return resolveDisplayName(firstNonEmpty_(ticket?.tecnico, ticket?.raw ? getRecordValueFinal(ticket.raw, ['TECNICO', 'Técnico', 'NOMBREUSUARIO', 'USUARIO', 'usuario']) : '', 'Sin usuario'));
}
function populateMetricsFilters(tickets) {
  const userSelect = $('metricsUserFilter');
  const statusSelect = $('metricsStatusFilter');
  if (!userSelect || !statusSelect) return;
  const baseUsers = new Set(getAllMetricUsersFinal());
  (tickets || []).forEach(ticket => { const name = getTicketMetricUser(ticket); if (name) baseUsers.add(name); });
  const excluded = getAnalyticsExcludedTechnicianKeys();
  const users = [...baseUsers].filter(name => !excluded.has(technicianKey(name))).sort((a, b) => a.localeCompare(b, 'es', { sensitivity: 'base' }));
  const currentUser = safeText(userSelect.value);
  const currentStatus = safeText(statusSelect.value);
  userSelect.innerHTML = '<option value="">Todos</option>' + users.map(user => `<option value="${escapeHtml(user)}">${escapeHtml(user)}</option>`).join('');
  statusSelect.innerHTML = '<option value="">Todos</option><option value="Cerrada">Cerrada</option><option value="Escalada">Escalada</option>';
  if (currentUser && users.includes(currentUser)) userSelect.value = currentUser;
  if (['Cerrada', 'Escalada'].includes(currentStatus)) statusSelect.value = currentStatus;
}
function filterTicketsForMetrics(tickets) {
  const { start, end } = getMetricsDateRange();
  const selectedUser = safeText($('metricsUserFilter')?.value);
  const selectedStatus = normalizeStatusLabel($('metricsStatusFilter')?.value);
  return (tickets || []).filter(ticket => {
    const date = getTicketMetricDate(ticket);
    const status = getTicketMetricStatus(ticket);
    const inRange = date >= start && date <= end;
    const userMatch = !selectedUser || getTicketMetricUser(ticket) === selectedUser;
    const statusMatch = selectedStatus ? status === selectedStatus : ['Cerrada', 'Escalada'].includes(status);
    return inRange && userMatch && statusMatch;
  });
}
function buildMetricsProductivityRows(tickets) {
  ...
  const excluded = getAnalyticsExcludedTechnicianKeys();
  const users = new Map();
  (selectedUser ? [selectedUser] : getAllMetricUsersFinal()).forEach(user => {
    if (!user || excluded.has(technicianKey(user))) return;
    users.set(user, { total: 0, escalados: 0, cerrados: 0, sinCierre: 0 });
  });
  (tickets || []).forEach(ticket => {
    const user = getTicketMetricUser(ticket) || 'Sin usuario';
    if (excluded.has(technicianKey(user))) return;
    if (!users.has(user)) users.set(user, { total: 0, escalados: 0, cerrados: 0, sinCierre: 0 });
    const stats = users.get(user);
    const status = getTicketMetricStatus(ticket);
    if (status === 'Escalada') { stats.escalados += 1; stats.total += 1; }
    else if (status === 'Cerrada') { stats.cerrados += 1; stats.total += 1; }
    else { stats.sinCierre += 1; }
  });
  return [...users.entries()].sort((a, b) => b[1].total - a[1].total || a[0].localeCompare(b[0], 'es', { sensitivity: 'base' })).map(([user, s]) => [user, String(s.total), String(s.escalados), String(s.cerrados), String(s.sinCierre)]);
}
function buildMetricsCharts(snapshot) {
  destroyAnalyticsCharts();
  const rows = (snapshot.productivityRows || []).filter(row => Number(row[1] || 0) > 0);
  const userLabels = rows.map(row => row[0]);
  const userTotals = rows.map(row => Number(row[1] || 0));
  const highlight = $('metricsTopPerformer');
  if (highlight) highlight.innerHTML = userTotals[0] ? `Usuario destacado: <strong>${escapeHtml(userLabels[0])}</strong> · ${userTotals[0]} ticket(s)` : 'Sin tickets cerrados o escalados para el periodo.';
  makeChart('metricsUsersChart', { type: 'bar', data: { labels: userLabels, datasets: [{ label: 'Tickets cerrados/escalados', data: userTotals, backgroundColor: userLabels.map((_, i) => i === 0 ? '#4f4744' : '#ada07c'), borderRadius: 8, borderSkipped: false }] }, options: { ...chartBaseOptions(), indexAxis: 'y', plugins: { legend: { display: false } }, scales: { x: { beginAtZero: true, ticks: { color: '#6b615c', precision: 0 }, grid: { color: 'rgba(79,71,68,0.12)' } }, y: { ticks: { color: '#4f4744', font: { family: 'Inter', size: 11, weight: '600' } }, grid: { display: false } } } } });
  const statusRows = snapshot.statusRows || [['Cerrada', '0'], ['Escalada', '0']];
  makeChart('metricsStatusChart', { type: 'doughnut', data: { labels: statusRows.map(r => r[0]), datasets: [{ data: statusRows.map(r => Number(r[1] || 0)), backgroundColor: getChartPalette(statusRows.length), borderColor: '#ffffff', borderWidth: 2 }] }, options: { responsive: true, maintainAspectRatio: false, animation: false, plugins: { legend: { position: 'bottom', labels: { color: '#4f4744', font: { family: 'Inter', size: 11, weight: '600' } } } } } });
  const timelineRows = snapshot.timelineRows || [];
  makeChart('metricsTimelineChart', { type: 'line', data: { labels: timelineRows.map(r => r[0]), datasets: [{ label: 'Tickets cerrados/escalados', data: timelineRows.map(r => Number(r[1] || 0)), borderColor: '#4f4744', backgroundColor: rgba('#ada07c', 0.25), fill: true, tension: 0.35, pointRadius: 4, pointBackgroundColor: '#8b8177' }] }, options: { ...chartBaseOptions(), plugins: { legend: { display: false } }, scales: { x: { ticks: { color: '#6b615c', maxRotation: 0, autoSkip: true }, grid: { color: 'rgba(79,71,68,0.12)' } }, y: { beginAtZero: true, ticks: { color: '#6b615c', precision: 0 }, grid: { color: 'rgba(79,71,68,0.12)' } } } } });
  const serviceRows = (snapshot.serviceRows || []).slice(0, 8);
  makeChart('metricsServiceChart', { type: 'bar', data: { labels: serviceRows.map(r => r[0]), datasets: [{ label: 'Tickets', data: serviceRows.map(r => Number(r[1] || 0)), backgroundColor: getChartPalette(serviceRows.length), borderRadius: 8, borderSkipped: false }] }, options: { ...chartBaseOptions(), plugins: { legend: { display: false } }, scales: { x: { ticks: { color: '#6b615c', maxRotation: 0, minRotation: 0, autoSkip: false, callback: function(value) { const label = this.getLabelForValue(value); return String(label).length > 12 ? String(label).slice(0, 12) + '…' : label; } }, grid: { display: false } }, y: { beginAtZero: true, ticks: { color: '#6b615c', precision: 0 }, grid: { color: 'rgba(79,71,68,0.12)' } } } } });
}
async function renderMetricsDashboard() {
  if (!isAnalyticsAllowedUser()) return;
  updateMetricsRangeFields();
  ...
  const filtered = filterTicketsForMetrics(allTickets);
  const productivityRows = buildMetricsProductivityRows(filtered);
  const cerrados = filtered.filter(ticket => getTicketMetricStatus(ticket) === 'Cerrada').length;
  const escalados = filtered.filter(ticket => getTicketMetricStatus(ticket) === 'Escalada').length;
  const serviceMap = new Map();
  filtered.forEach(ticket => {
    const service = safeText(ticket.servicio) || 'Sin servicio';
    serviceMap.set(service, (serviceMap.get(service) || 0) + 1);
  ...
  const timeline = buildTimelineGroups(filtered, range.mode, range.start, range.end);
  $('metricsTotalTickets').textContent = String(filtered.length);
  $('metricsEscalados').textContent = String(escalados);
  $('metricsResueltos').textContent = String(cerrados);
  $('metricsPendientes').textContent = String(getAllMetricUsersFinal().length);
  const statusRows = [['Cerrada', String(cerrados)], ['Escalada', String(escalados)]];
  const timelineRows = timeline.labels.map((label, i) => [label, String(timeline.values[i] || 0)]);
  const serviceRows = buildSimpleCountRows(serviceMap, 'Sin servicio');
  renderSimpleMetricsTable('metricsUsersTableBody', productivityRows, 5);
  APP.metricsSnapshot = { filtered, productivityRows, statusRows, timelineRows, serviceRows, cerrados, escalados, usuariosBase: getAllMetricUsersFinal().length, rangeText: `${range.start.toLocaleDateString('es-MX')} - ${range.end.toLocaleDateString('es-MX')}` };
  buildMetricsCharts(APP.metricsSnapshot);
}
window.renderMetricsDashboard = renderMetricsDashboard;
async function exportMetricsPDF() {
  ...
  try {
    ...
    doc.autoTable({
      ...
      head: [['Indicador', 'Valor']],
      body: [
        ['Total tickets cerrados/escalados', String(snapshot.filtered.length)],
        ['Escalados', String(snapshot.escalados || 0)],
        ['Cerrados', String(snapshot.cerrados || 0)],
        ['Usuarios en base', String(snapshot.usuariosBase || 0)]
      ],
      margin: { left: 15, right: 15 }
    });
    let y = doc.lastAutoTable.finalY + 8;
    ...
    doc.autoTable({
      ...
      styles: { fontSize: 8.5, cellPadding: 4, lineColor: PDF_PALETTE.line, textColor: PDF_PALETTE.text, font: 'times' },
      headStyles: { fillColor: PDF_PALETTE.taupe, textColor: [255,255,255], font: 'times', fontStyle: 'bold' },
      head: [['Usuario', 'Total', 'Escalados', 'Cerrados', 'Sin cierre']],
      body: snapshot.productivityRows.length ? snapshot.productivityRows : [['Sin datos', '0', '0', '0', '0']],
      margin: { left: 15, right: 15 }
    });
    const totalPages = doc.internal.getNumberOfPages();
...
/* =========================================================
   AJUSTE FINAL MARIO - MÉTRICAS EN PROCESO + GUARDADO RÁPIDO
   ========================================================= */
function evidenciaTieneReferenciaRemotaFinal(item) {
  const normalized = normalizeEvidenceItem(item, 0);
  if (!normalized) return false;
  const candidates = [
    normalized.sourceUrl,
    normalized.previewUrl,
    ...(Array.isArray(normalized.previewCandidates) ? normalized.previewCandidates : [])
  ].map(safeText).filter(Boolean);
  return candidates.some(value => {
    if (!value || value.startsWith('data:image')) return false;
    if (value.startsWith('blob:')) return false;
    return /^https?:\/\//i.test(value) || /^[a-zA-Z0-9_-]{20,}$/.test(value);
  });
}

function obtenerReferenciaEvidenciaFinal(item) {
  const normalized = normalizeEvidenceItem(item, 0);
  ...
  return candidates.find(value => value && !value.startsWith('data:image') && !value.startsWith('blob:')) || '';
}

function buildExistingEvidenceReferenceMetaFinal(evidencias = []) {
  const source = Array.isArray(evidencias) ? evidencias.map((item, index) => normalizeEvidenceItem(item, index)).filter(Boolean) : [];
  const existing = source.filter(evidenciaTieneReferenciaRemotaFinal);
  const names = [];
  const ids = [];
  const links = [];
  const viewLinks = [];
  ...
  existing.forEach((item, index) => {
    const ref = obtenerReferenciaEvidenciaFinal(item);
    const id = extractDriveFileId(ref) || (/^[a-zA-Z0-9_-]{20,}$/.test(ref) ? ref : '');
    const name = safeText(item.name) || `evidencia_existente_${index + 1}.jpg`;
    names.push(name);
    if (id) ids.push(id);
    if (ref) links.push(ref);
    const view = id ? `https://drive.google.com/file/d/${id}/view?usp=sharing` : ref;
    const thumb = id ? `https://drive.google.com/thumbnail?id=${id}&sz=w1600` : safeText(item.previewUrl || ref);
    if (view) viewLinks.push(view);
    if (thumb) thumbLinks.push(thumb);
  });
  ...
  return {
    ...
    viewLinksJoined: viewLinks.join(' | '),
    thumbLinksJoined: thumbLinks.join(' | ')
  };
}

async function buildCompressedEvidenceForApi(evidencias = []) {
  const source = Array.isArray(evidencias) ? evidencias.map((item, index) => normalizeEvidenceItem(item, index)).filter(Boolean) : [];
  const newImages = source.filter(item => {
    const dataUrl = safeText(item.dataUrl);
    if (!dataUrl.startsWith('data:image')) return false;
    return !evidenciaTieneReferenciaRemotaFinal(item);
  });
  ...
  for (let i = 0; i < newImages.length; i++) {
    ...
    const jpgDataUrl = await compressImageDataUrl(item.dataUrl, 1280, 0.78);
    compressed.push({
      name: safeText(item.name) ? safeText(item.name).replace(/\.[^.]+$/, '.jpg') : `evidencia_nueva_${i + 1}.jpg`,
      dataUrl: jpgDataUrl
    });
  }
  return compressed;
...
async function buildApiPayloadForSave(data) {
  const payload = buildApiPayload(data);
  const existingMeta = buildExistingEvidenceReferenceMetaFinal(data.evidencias || []);
  const compressedEvidence = await buildCompressedEvidenceForApi(data.evidencias || []);
  const newNames = compressedEvidence.map(item => item.name);
  const allNames = [...existingMeta.names, ...newNames];
  const totalCount = existingMeta.count + compressedEvidence.length;

  payload.evidencias = compressedEvidence;
  payload.fotosDiagnostico = compressedEvidence;
  payload.fotosSolucion = [];

  payload.EVIDENCIASCOUNT = totalCount;
  payload.EVIDENCIASNOMBRES = allNames.join(' | ');
  payload.EVIDENCIASIDS = existingMeta.idsJoined;
  payload.EVIDENCIASLINKS = existingMeta.linksJoined;
  payload.EVIDENCIASVIEWLINKS = existingMeta.viewLinksJoined;
  payload.EVIDENCIASTHUMBLINKS = existingMeta.thumbLinksJoined;

  payload.FOTOSDIAGNOSTICOCOUNT = totalCount;
  payload.FOTOSDIAGNOSTICONOMBRES = payload.EVIDENCIASNOMBRES;
  payload.FOTOSDIAGNOSTICOIDS = existingMeta.idsJoined;
  payload.FOTOSDIAGNOSTICOLINKS = existingMeta.linksJoined;
  payload.FOTOSDIAGNOSTICOVIEWLINKS = existingMeta.viewLinksJoined;
  payload.FOTOSDIAGNOSTICOTHUMBLINKS = existingMeta.thumbLinksJoined;
  payload.FOTOSSOLUCIONCOUNT = 0;
  payload.FOTOSSOLUCIONNOMBRES = '';

  payload.MANTENER_EVIDENCIAS_EXISTENTES = true;
  payload.SOLO_ACTUALIZAR_DATOS = compressedEvidence.length === 0;
  payload.SIN_REUPLOAD_EVIDENCIAS = compressedEvidence.length === 0;
  return payload;
}

function getTicketMetricStatus(ticket) {
  return normalizeStatusLabel(getTicketStatus(ticket)) || 'En proceso';
}
...
function populateMetricsFilters(tickets) {
  ...
  (tickets || []).forEach(ticket => {
    const name = getTicketMetricUser(ticket);
    if (name) baseUsers.add(name);
  });
  const excluded = getAnalyticsExcludedTechnicianKeys();
  const users = [...baseUsers]
    .filter(name => !excluded.has(technicianKey(name)))
    .sort((a, b) => a.localeCompare(b, 'es', { sensitivity: 'base' }));
  const currentUser = safeText(userSelect.value);
  const currentStatus = safeText(statusSelect.value);
  userSelect.innerHTML = '<option value="">Todos</option>' + users.map(user => `<option value="${escapeHtml(user)}">${escapeHtml(user)}</option>`).join('');
  statusSelect.innerHTML = '<option value="">Todos</option><option value="Cerrada">Cerrada</option><option value="Escalada">Escalada</option><option value="En proceso">En proceso</option>';
  if (currentUser && users.includes(currentUser)) userSelect.value = currentUser;
  if (['Cerrada', 'Escalada', 'En proceso'].includes(currentStatus)) statusSelect.value = currentStatus;
}

function filterTicketsForMetrics(tickets) {
  const { start, end } = getMetricsDateRange();
  const selectedUser = safeText($('metricsUserFilter')?.value);
  const selectedStatus = safeText($('metricsStatusFilter')?.value);
  return (tickets || []).filter(ticket => {
    const date = getTicketMetricDate(ticket);
    const status = getTicketMetricStatus(ticket);
    const inRange = date >= start && date <= end;
...
function buildMetricsProductivityRows(tickets) {
  ...
  (selectedUser ? [selectedUser] : getAllMetricUsersFinal()).forEach(user => {
    if (!user || excluded.has(technicianKey(user))) return;
    users.set(user, { total: 0, escalados: 0, cerrados: 0, enProceso: 0 });
  });
  (tickets || []).forEach(ticket => {
    const user = getTicketMetricUser(ticket) || 'Sin usuario';
    if (excluded.has(technicianKey(user))) return;
    if (!users.has(user)) users.set(user, { total: 0, escalados: 0, cerrados: 0, enProceso: 0 });
    const stats = users.get(user);
    const status = getTicketMetricStatus(ticket);
    stats.total += 1;
    if (status === 'Escalada') stats.escalados += 1;
    else if (status === 'Cerrada') stats.cerrados += 1;
    else stats.enProceso += 1;
  });
  return [...users.entries()]
    .sort((a, b) => b[1].total - a[1].total || a[0].localeCompare(b[0], 'es', { sensitivity: 'base' }))
    .map(([user, s]) => [user, String(s.total), String(s.escalados), String(s.cerrados), String(s.enProceso)]);
}
...
function buildMetricsCharts(snapshot) {
  ...
  const highlight = $('metricsTopPerformer');
  if (highlight) {
    highlight.innerHTML = userTotals[0]
      ? `Usuario destacado: <strong>${escapeHtml(userLabels[0])}</strong> · ${userTotals[0]} ticket(s) en el periodo.`
      : 'Sin tickets para el periodo. La tabla conserva todos los usuarios con 0.';
  }
  makeChart('metricsUsersChart', {
    type: 'bar',
    data: { labels: userLabels, datasets: [{ label: 'Tickets', data: userTotals, backgroundColor: userLabels.map((_, i) => i === 0 ? '#4f4744' : '#ada07c'), borderRadius: 8, borderSkipped: false }] },
    options: { ...chartBaseOptions(), indexAxis: 'y', plugins: { legend: { display: false } }, scales: { x: { beginAtZero: true, ticks: { color: '#6b615c', precision: 0 }, grid: { color: 'rgba(79,71,68,0.12)' } }, y: { ticks: { color: '#4f4744', font: { family: 'Inter', size: 11, weight: '600' } }, grid: { display: false } } } }
  });
  const statusRows = snapshot.statusRows || [['Cerrada', '0'], ['Escalada', '0'], ['En proceso', '0']];
  makeChart('metricsStatusChart', {
    type: 'doughnut',
    data: { labels: statusRows.map(r => r[0]), datasets: [{ data: statusRows.map(r => Number(r[1] || 0)), backgroundColor: getChartPalette(statusRows.length), borderColor: '#ffffff', borderWidth: 2 }] },
    options: { responsive: true, maintainAspectRatio: false, animation: false, plugins: { legend: { position: 'bottom', labels: { color: '#4f4744', font: { family: 'Inter', size: 11, weight: '600' } } } } }
  });
  const timelineRows = snapshot.timelineRows || [];
  makeChart('metricsTimelineChart', {
    type: 'line',
    data: { labels: timelineRows.map(r => r[0]), datasets: [{ label: 'Tickets', data: timelineRows.map(r => Number(r[1] || 0)), borderColor: '#4f4744', backgroundColor: rgba('#ada07c', 0.25), fill: true, tension: 0.35, pointRadius: 4, pointBackgroundColor: '#8b8177' }] },
    options: { ...chartBaseOptions(), plugins: { legend: { display: false } }, scales: { x: { ticks: { color: '#6b615c', maxRotation: 0, autoSkip: true }, grid: { color: 'rgba(79,71,68,0.12)' } }, y: { beginAtZero: true, ticks: { color: '#6b615c', precision: 0 }, grid: { color: 'rgba(79,71,68,0.12)' } } } }
  });
  const serviceRows = (snapshot.serviceRows || []).slice(0, 8);
  makeChart('metricsServiceChart', {
    type: 'bar',
    data: { labels: serviceRows.map(r => r[0]), datasets: [{ label: 'Tickets', data: serviceRows.map(r => Number(r[1] || 0)), backgroundColor: getChartPalette(serviceRows.length), borderRadius: 8, borderSkipped: false }] },
...
async function renderMetricsDashboard() {
  ...
  const cerrados = filtered.filter(ticket => getTicketMetricStatus(ticket) === 'Cerrada').length;
  const escalados = filtered.filter(ticket => getTicketMetricStatus(ticket) === 'Escalada').length;
  const enProceso = filtered.filter(ticket => getTicketMetricStatus(ticket) === 'En proceso').length;
  const serviceMap = new Map();
  filtered.forEach(ticket => {
    const service = safeText(ticket.servicio) || 'Sin servicio';
    serviceMap.set(service, (serviceMap.get(service) || 0) + 1);
  ...
  $('metricsEscalados').textContent = String(escalados);
  $('metricsResueltos').textContent = String(cerrados);
  $('metricsPendientes').textContent = String(enProceso);
  const statusRows = [['Cerrada', String(cerrados)], ['Escalada', String(escalados)], ['En proceso', String(enProceso)]];
  const timelineRows = timeline.labels.map((label, i) => [label, String(timeline.values[i] || 0)]);
  const serviceRows = buildSimpleCountRows(serviceMap, 'Sin servicio');
  renderSimpleMetricsTable('metricsUsersTableBody', productivityRows, 5);
  APP.metricsSnapshot = {
    ...
    escalados,
    enProceso,
    usuariosBase: getAllMetricUsersFinal().length,
    rangeText: `${range.start.toLocaleDateString('es-MX')} - ${range.end.toLocaleDateString('es-MX')}`
  };
  buildMetricsCharts(APP.metricsSnapshot);
}
  ...
  try {
    ...
    doc.autoTable({
      ...
      body: [
        ...
        ['Escalados', String(snapshot.escalados || 0)],
        ['Cerrados', String(snapshot.cerrados || 0)],
        ['En proceso', String(snapshot.enProceso || 0)],
        ['Usuarios en base', String(snapshot.usuariosBase || 0)]
      ],
      margin: { left: 15, right: 15 }
    });
    ...
    doc.autoTable({
      ...
      styles: { fontSize: 8.5, cellPadding: 4, lineColor: PDF_PALETTE.line, textColor: PDF_PALETTE.text, font: 'times' },
      headStyles: { fillColor: PDF_PALETTE.taupe, textColor: [255,255,255], font: 'times', fontStyle: 'bold' },
      head: [['Usuario', 'Total', 'Escalados', 'Cerrados', 'En proceso']],
      body: snapshot.productivityRows.length ? snapshot.productivityRows : [['Sin datos', '0', '0', '0', '0']],
      margin: { left: 15, right: 15 }
    });
    const totalPages = doc.internal.getNumberOfPages();
...
/* =========================================================
   AJUSTE FINAL MARIO - EVIDENCIAS DRIVE + STATUS EN PROCESO
   ========================================================= */
function getRecordRawValueFinal(record, names = []) {
  if (!record || typeof record !== 'object') return undefined;
  const keys = Object.keys(record);
  const targets = names.map(normalizeFieldKeyFinal);
  for (const target of targets) {
    const key = keys.find(k => normalizeFieldKeyFinal(k) === target);
    if (key !== undefined && record[key] !== undefined && record[key] !== null && String(record[key]).trim() !== '') return record[key];
  }
  for (const target of targets) {
    const key = keys.find(k => {
      const kk = normalizeFieldKeyFinal(k);
...
function parseMaybeJsonFinal(value) {
...
function splitJoinedValues(value) {
  const parsed = parseMaybeJsonFinal(value);
  if (Array.isArray(parsed)) return parsed.map(item => typeof item === 'object' ? item : safeText(item)).filter(Boolean);
  if (parsed && typeof parsed === 'object') return [parsed];
  const raw = safeText(parsed);
  if (!raw) return [];
  return raw
    .split(/\s*\|\s*|\n+|\r+|;+/i)
...
function looksLikeDriveOrImageRefFinal(value) {
  const raw = safeText(value);
  if (!raw) return false;
...
function uniqueEvidenceKeyFinal(item) {
  const normalized = normalizeEvidenceItem(item, 0) || {};
  const id = extractDriveFileId(normalized.sourceUrl || normalized.previewUrl || '') || extractDriveFileId((normalized.previewCandidates || []).join(' '));
  return safeText(id || normalized.sourceUrl || normalized.previewUrl || normalized.name).toLowerCase();
}

function mergeEvidenceListsFinal() {
  const map = new Map();
  Array.from(arguments).flat().forEach((item, index) => {
    const normalized = normalizeEvidenceItem(item, index);
    if (!normalized) return;
    const hasRef = safeText(normalized.dataUrl || normalized.previewUrl || normalized.sourceUrl) || (normalized.previewCandidates || []).length;
    if (!hasRef) return;
    const key = uniqueEvidenceKeyFinal(normalized) || `evidencia_${map.size}_${index}`;
    if (!map.has(key)) {
      ...
      map.set(key, {
        ...current,
        ...normalized,
        dataUrl: current.dataUrl || normalized.dataUrl || '',
        previewUrl: current.previewUrl || normalized.previewUrl || '',
        sourceUrl: current.sourceUrl || normalized.sourceUrl || '',
        previewCandidates: [...new Set([...(current.previewCandidates || []), ...(normalized.previewCandidates || [])].filter(Boolean))]
      });
    }
  });
  return [...map.values()];
...
function normalizeEvidenceItem(item, index = 0) {
  ...
  if (typeof item === 'string') {
    ...
    const candidates = buildEvidencePreviewCandidates(value);
    return {
      name: `evidencia_${index + 1}.jpg`,
      dataUrl: value.startsWith('data:image') ? value : '',
      sourceUrl: value.startsWith('data:image') ? '' : value,
      previewCandidates: candidates,
      previewUrl: candidates[0] || ''
    };
  }
  if (typeof item !== 'object') return null;
  const rawId = safeText(item.id || item.fileId || item.driveId || item.ID || item.FILEID || item.file_id || item.drive_id);
  const rawUrl = safeText(
    item.sourceUrl || item.url || item.URL || item.link || item.LINK || item.href || item.fileUrl || item.fileURL ||
    item.viewUrl || item.webViewLink || item.webContentLink || item.previewUrl || item.thumbUrl || item.thumbnailUrl ||
    item.downloadUrl || item.ruta || item.RUTA || item.rutas || item.RUTAS || item.archivo || item.ARCHIVO ||
    item.archivoUrl || item.imagen || item.imagenUrl || item.evidencia || item.EVIDENCIA || item.linkDrive || item.driveUrl || ''
  );
  const dataUrl = safeText(item.dataUrl || item.base64 || item.data || item.imagenBase64 || '');
  const previewCandidates = Array.isArray(item.previewCandidates) && item.previewCandidates.length
    ? item.previewCandidates.map(safeText).filter(Boolean)
    : buildEvidencePreviewCandidates(item.thumbUrl || item.thumbnailUrl || item.previewUrl || item.viewUrl || rawUrl || rawId || dataUrl);
  return {
    name: safeText(item.name || item.nombre || item.filename || item.fileName || item.NOMBRE || item.FILENAME) || `evidencia_${index + 1}.${String(dataUrl).includes('image/png') ? 'png' : 'jpg'}`,
    dataUrl: dataUrl.startsWith('data:image') ? dataUrl : '',
    sourceUrl: dataUrl.startsWith('data:image') ? rawUrl : (rawUrl || rawId),
    previewCandidates,
    previewUrl: safeText(item.previewUrl || item.thumbUrl || item.thumbnailUrl || item.viewUrl || previewCandidates[0] || '')
  };
}

function extractEvidenceObjectsFromValueFinal(value) {
  const parsed = parseMaybeJsonFinal(value);
  if (!parsed) return [];
  if (Array.isArray(parsed)) return parsed;
  if (typeof parsed === 'object') {
    if (Array.isArray(parsed.evidencias)) return parsed.evidencias;
    if (Array.isArray(parsed.fotosDiagnostico)) return parsed.fotosDiagnostico;
    if (Array.isArray(parsed.files)) return parsed.files;
    if (Array.isArray(parsed.archivos)) return parsed.archivos;
    return [parsed];
  }
  return splitJoinedValues(parsed).filter(looksLikeDriveOrImageRefFinal);
}

function extractEvidenceFromRecord(record) {
  if (!record || typeof record !== 'object') return [];
  const objectFields = [
    'EVIDENCIAS', 'Evidencias', 'evidencias', 'EVIDENCIASJSON', 'EVIDENCIAS JSON', 'EVIDENCIAS_JSON',
    'FOTOSDIAGNOSTICO', 'FOTOS DIAGNOSTICO', 'FOTOS_DIAGNOSTICO', 'fotosDiagnostico',
    'IMAGENES', 'IMÁGENES', 'Imagenes', 'imagenes', 'ARCHIVOS', 'Archivos', 'archivos',
    'RUTAS', 'Rutas', 'rutas', 'LINKS', 'Links', 'links'
  ];
  const objectItems = [];
  objectFields.forEach(name => objectItems.push(...extractEvidenceObjectsFromValueFinal(getRecordRawValueFinal(record, [name]))));

  const names = [
    ...splitJoinedValues(getRecordRawValueFinal(record, ['EVIDENCIASNOMBRES', 'EVIDENCIAS NOMBRES', 'NOMBRES EVIDENCIAS', 'evidenciasNombres'])),
    ...splitJoinedValues(getRecordRawValueFinal(record, ['FOTOSDIAGNOSTICONOMBRES', 'FOTOS DIAGNOSTICO NOMBRES', 'fotosDiagnosticoNombres']))
  ].map(item => typeof item === 'object' ? safeText(item.name || item.nombre || item.fileName) : safeText(item)).filter(Boolean);

  const ids = [
    ...splitJoinedValues(getRecordRawValueFinal(record, ['EVIDENCIASIDS', 'EVIDENCIAS IDS', 'EVIDENCIA IDS', 'IDS EVIDENCIAS', 'evidenciasIds', 'IDS'])),
    ...splitJoinedValues(getRecordRawValueFinal(record, ['FOTOSDIAGNOSTICOIDS', 'FOTOS DIAGNOSTICO IDS', 'fotosDiagnosticoIds']))
  ].map(item => typeof item === 'object' ? safeText(item.id || item.fileId || item.driveId) : safeText(item)).filter(Boolean);

  const thumbs = [
    ...splitJoinedValues(getRecordRawValueFinal(record, ['EVIDENCIASTHUMBLINKS', 'EVIDENCIAS THUMB LINKS', 'EVIDENCIAS THUMBNAILS', 'THUMBNAILS EVIDENCIAS', 'evidenciasThumbLinks'])),
    ...splitJoinedValues(getRecordRawValueFinal(record, ['FOTOSDIAGNOSTICOTHUMBLINKS', 'FOTOS DIAGNOSTICO THUMB LINKS', 'fotosDiagnosticoThumbLinks']))
  ].map(item => typeof item === 'object' ? safeText(item.thumbUrl || item.thumbnailUrl || item.url) : safeText(item)).filter(Boolean);

  const views = [
    ...splitJoinedValues(getRecordRawValueFinal(record, ['EVIDENCIASVIEWLINKS', 'EVIDENCIAS VIEW LINKS', 'EVIDENCIAS VER', 'VIEW LINKS EVIDENCIAS', 'evidenciasViewLinks'])),
    ...splitJoinedValues(getRecordRawValueFinal(record, ['FOTOSDIAGNOSTICOVIEWLINKS', 'FOTOS DIAGNOSTICO VIEW LINKS', 'fotosDiagnosticoViewLinks']))
  ].map(item => typeof item === 'object' ? safeText(item.viewUrl || item.webViewLink || item.url) : safeText(item)).filter(Boolean);

  const links = [
    ...splitJoinedValues(getRecordRawValueFinal(record, ['EVIDENCIASLINKS', 'EVIDENCIAS LINKS', 'LINKS EVIDENCIAS', 'RUTAS EVIDENCIAS', 'evidenciasLinks'])),
    ...splitJoinedValues(getRecordRawValueFinal(record, ['FOTOSDIAGNOSTICOLINKS', 'FOTOS DIAGNOSTICO LINKS', 'RUTAS FOTOS DIAGNOSTICO', 'fotosDiagnosticoLinks'])),
    ...splitJoinedValues(getRecordRawValueFinal(record, ['RUTASIMAGENES', 'RUTAS IMAGENES', 'RUTAS IMÁGENES', 'LINKSIMAGENES', 'LINKS IMAGENES', 'URLSIMAGENES', 'URLS IMAGENES']))
  ].map(item => typeof item === 'object' ? safeText(item.url || item.link || item.sourceUrl || item.viewUrl) : safeText(item)).filter(Boolean);
  ...
  for (let i = 0; i < total; i++) {
    structured.push({
      name: safeText(names[i]) || `Evidencia ${i + 1}`,
      id: safeText(ids[i]),
      thumbUrl: safeText(thumbs[i]),
      previewUrl: safeText(thumbs[i] || views[i] || links[i]),
      viewUrl: safeText(views[i]),
      sourceUrl: safeText(links[i] || views[i] || thumbs[i] || ids[i])
    });
  }
  return mergeEvidenceListsFinal(objectItems, structured);
}

function getTicketDisplayStatusFinal(ticket) {
  return normalizeStatusLabel(getTicketStatus(ticket)) || 'En proceso';
}
function statusBadgeClassFinal(status) {
  const s = getTicketDisplayStatusFinal({ status });
  if (s === 'Cerrada') return 'status-cerrada';
  if (s === 'Escalada') return 'status-escalada';
  return 'status-proceso';
}
function ticketEvidenceCountFinal(ticket) {
  return mergeEvidenceListsFinal(ticket?.evidencias || [], ticket?.raw ? extractEvidenceFromRecord(ticket.raw) : []).length;
}
function firstEvidencePreviewFinal(ticket) {
  const evidencias = mergeEvidenceListsFinal(ticket?.evidencias || [], ticket?.raw ? extractEvidenceFromRecord(ticket.raw) : []);
  const first = evidencias[0] ? normalizeEvidenceItem(evidencias[0], 0) : null;
  return first ? safeText(first.previewUrl || first.sourceUrl || (first.previewCandidates || [])[0]) : '';
}
function enrichTicketEvidenceFinal(ticket) {
  if (!ticket) return ticket;
  const rawEvidencias = ticket.raw ? extractEvidenceFromRecord(ticket.raw) : [];
  return { ...ticket, status: normalizeStatusLabel(ticket.status), evidencias: mergeEvidenceListsFinal(ticket.evidencias || [], rawEvidencias) };
}

function normalizeRemoteTicket(record, index) {
  let parsedChecklist = { checklist1: [], checklist2: [] };
  try {
    const parsed = JSON.parse(getRecordValueFinal(record, ['CHECKLIST', 'checklist', 'CHECKLIST1', 'CHECKLIST 1']) || '');
    if (parsed && typeof parsed === 'object') parsedChecklist = { checklist1: parsed.checklist1 || parsed.CHECKLIST1 || [], checklist2: parsed.checklist2 || parsed.CHECKLIST2 || [] };
  } catch(e) {}
  let dynamicFields = [];
  try {
    const parsed = JSON.parse(getRecordValueFinal(record, ['CAMPOSDINAMICOS', 'CAMPOS DINAMICOS', 'CAMPOS DINÁMICOS', 'camposDinamicos']) || '[]');
    if (Array.isArray(parsed)) dynamicFields = parsed;
  } catch(e) {}
  if (!dynamicFields.length) {
    const flatPairs = [
      ['casoArea', 'Área', getRecordValueFinal(record, ['AREAUSUARIO', 'AREA USUARIO', 'ÁREA', 'AREA', 'Area'])],
      ['casoEquipo', 'Equipo', getRecordValueFinal(record, ['TIPODEEQUIPO', 'TIPO DE EQUIPO', 'EQUIPO', 'Equipo'])],
      ['casoSerie', 'No. Serie', getRecordValueFinal(record, ['NUMEROSERIE', 'NUMERO SERIE', 'NUMERO DE SERIE', 'No Serie', 'NO SERIE'])],
      ['casoDescripcion', 'Descripción del problema', getRecordValueFinal(record, ['DESCRIPCIONBREVE', 'DESCRIPCION BREVE', 'DESCRIPCION', 'DESCRIPCIÓN', 'Descripción'])],
      ['casoDiagnostico', 'Diagnóstico', getRecordValueFinal(record, ['DIAGNOSTICO', 'DIAGNÓSTICO', 'Diagnóstico'])],
      ['casoComentario', 'Comentario', getRecordValueFinal(record, ['SOLUCION', 'SOLUCIÓN', 'COMENTARIO', 'Comentario'])]
    ];
    dynamicFields = flatPairs.filter(([, , value]) => safeText(value)).map(([id, label, value]) => ({ id, label, value, required: false }));
  }
  const status = normalizeStatusLabel(firstNonEmpty_(
    findDynamicValue({ dynamicFields }, ['casoStatusSolicitud'], ['status de la solicitud', 'estatus de la solicitud', 'status solicitud']),
    getRecordValueFinal(record, ['STATUSSOLICITUD', 'STATUS SOLICITUD', 'ESTATUSSOLICITUD', 'ESTATUS DE LA SOLICITUD', 'STATUS', 'ESTATUS', 'Status de la solicitud', 'Estatus'])
  ));
  const escaladaArea = safeText(firstNonEmpty_(
    findDynamicValue({ dynamicFields }, ['casoAreaEscalada'], ['área a la que se escaló', 'area a la que se escalo', 'area escalada']),
    getRecordValueFinal(record, ['AREAESCALADA', 'AREA ESCALADA', 'ÁREA A LA QUE SE ESCALÓ', 'AREA A LA QUE SE ESCALO'])
  ));
  const tienda = getRecordValueFinal(record, ['SUCURSAL', 'TIENDA', 'SUCURSALTIENDA', 'NOMBRETIENDA', 'NOMBRE TIENDA', 'Tienda', 'Sucursal', 'Sucursal o tienda', 'Sitio o tienda', 'NOMBRE COMERCIAL']);
  return enrichTicketEvidenceFinal({
    id: `api:${index}`,
    source: 'api',
    folio: getRecordValueFinal(record, ['FOLIO', 'Folio']),
    fecha: normalizeDateOnly(getRecordValueFinal(record, ['FECHA', 'Fecha'])),
    tecnico: resolveDisplayName(getRecordValueFinal(record, ['TECNICO', 'Técnico', 'NOMBREUSUARIO', 'NOMBRE USUARIO', 'USUARIO', 'usuario'])),
    marca: getRecordValueFinal(record, ['MARCA', 'Marca']),
    tienda,
    servicio: getRecordValueFinal(record, ['TIPOSERVICIO', 'TIPO SERVICIO', 'TIPO DE SERVICIO', 'Tipo de servicio', 'SERVICIO', 'Servicio']),
    status,
    escaladaArea,
    dynamicFields,
    checklists: parsedChecklist,
    evidencias: extractEvidenceFromRecord(record),
    updatedAt: Date.parse(getRecordValueFinal(record, ['timestamp', 'TIMESTAMP', 'ACTUALIZADO', 'FECHA']) || '') || Date.now(),
    createdAt: Date.parse(getRecordValueFinal(record, ['timestamp', 'TIMESTAMP', 'CREADO', 'FECHA']) || '') || Date.now(),
    raw: record
  });
}
...
function mergeTickets(remoteTickets, localTickets) {
  const map = new Map();
  (remoteTickets || []).forEach(ticket => {
    const enriched = enrichTicketEvidenceFinal(ticket);
    const key = `${safeText(enriched.folio).toLowerCase()}|${normalizeDateOnly(enriched.fecha).toLowerCase()}`;
    if (key !== '|') map.set(key, enriched);
  });
  (localTickets || []).forEach(ticket => {
    const key = `${safeText(ticket.folio).toLowerCase()}|${normalizeDateOnly(ticket.fecha).toLowerCase()}`;
    const remoteTicket = map.get(key);
    const merged = enrichTicketEvidenceFinal({
      ...(remoteTicket || {}),
      ...ticket,
      id: `local:${ticket.id}`,
      source: 'local',
      ...
      status: normalizeStatusLabel(firstNonEmpty_(ticket.status, remoteTicket?.status)),
      escaladaArea: safeText(firstNonEmpty_(ticket.escaladaArea, remoteTicket?.escaladaArea)),
      dynamicFields: (Array.isArray(ticket.dynamicFields) && ticket.dynamicFields.length) ? ticket.dynamicFields : (remoteTicket?.dynamicFields || []),
      checklists: (ticket.checklists && ((ticket.checklists.checklist1 || []).length || (ticket.checklists.checklist2 || []).length)) ? ticket.checklists : (remoteTicket?.checklists || { checklist1: [], checklist2: [] }),
      raw: remoteTicket?.raw || ticket.raw || null,
      evidencias: mergeEvidenceListsFinal(ticket.evidencias || [], remoteTicket?.evidencias || [], remoteTicket?.raw ? extractEvidenceFromRecord(remoteTicket.raw) : [])
    });
    if (key === '|') map.set(`local:${ticket.id}`, merged);
...
async function getTicketByRef(ref) {
  const cleanRef = safeText(ref);
  const fromMerged = (APP.mergedTickets || []).find(ticket => ticket.id === cleanRef);
  if (fromMerged) return enrichTicketEvidenceFinal(fromMerged);
  if (cleanRef.startsWith('api:')) {
    const fromRemote = (APP.remoteTickets || []).find(ticket => ticket.id === cleanRef);
    return fromRemote ? enrichTicketEvidenceFinal(fromRemote) : null;
  }
  if (cleanRef.startsWith('local:')) {
    const id = cleanRef.split(':')[1];
    const local = await getLocalTicket(id);
    if (!local) return null;
    let remote = (APP.remoteTickets || []).find(ticket => safeText(ticket.folio) && safeText(ticket.folio) === safeText(local.folio) && normalizeDateOnly(ticket.fecha) === normalizeDateOnly(local.fecha));
    if (!remote) {
      try {
        APP.remoteTickets = await fetchTicketsFromAPI();
        remote = (APP.remoteTickets || []).find(ticket => safeText(ticket.folio) && safeText(ticket.folio) === safeText(local.folio) && normalizeDateOnly(ticket.fecha) === normalizeDateOnly(local.fecha));
      } catch (error) {}
    }
    const merged = mergeTickets(remote ? [remote] : [], [local]).find(ticket => ticket.id === cleanRef) || { ...local, id: cleanRef, source: 'local' };
    return enrichTicketEvidenceFinal(merged);
  }
  return null;
}

function renderTicketsListStatusCellFinal(ticket) {
  const status = getTicketDisplayStatusFinal(ticket);
  return `<span class="ticket-status-badge ${statusBadgeClassFinal(status)}">${escapeHtml(status)}</span>`;
}
function renderTicketsListEvidenceCellFinal(ticket) {
  const count = ticketEvidenceCountFinal(ticket);
  if (!count) return '<span class="ticket-evidence-pill empty">0 imágenes</span>';
  const preview = firstEvidencePreviewFinal(ticket);
  const img = preview ? `<img class="ticket-evidence-thumb" src="${escapeHtml(preview)}" alt="Evidencia" loading="lazy">` : '';
  return `<span class="ticket-evidence-pill">${img}${count} ${count === 1 ? 'imagen' : 'imágenes'}</span>`;
}

async function renderTicketsList() {
  const [remoteTickets, localTickets] = await Promise.all([fetchTicketsFromAPI(), getAllLocalTickets()]);
  APP.remoteTickets = remoteTickets.map(enrichTicketEvidenceFinal);
  APP.mergedTickets = mergeTickets(APP.remoteTickets, localTickets);
  const search = safeText($('ticketSearch')?.value).toLowerCase();
  const filtered = APP.mergedTickets.filter(ticket => {
    if (!ticketBelongsToCurrentUser(ticket)) return false;
    const text = [ticket.folio, ticket.tienda, ticket.servicio, ticket.tecnico, getTicketDisplayStatusFinal(ticket)].join(' ').toLowerCase();
    return !search || text.includes(search);
  });
  const tbody = $('ticketsTableBody');
  if (!tbody) return;
  tbody.innerHTML = filtered.length ? filtered.map(ticket => `
    <tr>
      <td>${escapeHtml(ticket.folio)}</td>
      <td>${escapeHtml(normalizeDateOnly(ticket.fecha))}</td>
      <td>${escapeHtml(resolveDisplayName(ticket.tecnico || APP.currentUser))}</td>
      <td>${escapeHtml(ticket.tienda)}</td>
      <td>${escapeHtml(ticket.servicio)}</td>
      <td>${renderTicketsListStatusCellFinal(ticket)}</td>
      <td>${renderTicketsListEvidenceCellFinal(ticket)}</td>
      <td>
        <div class="table-actions">
          <button class="action-btn" type="button" data-action="edit" data-ref="${escapeHtml(ticket.id)}"><i class="fas fa-pen"></i> Editar</button>
          <button class="action-btn" type="button" data-action="pdf" data-ref="${escapeHtml(ticket.id)}"><i class="fas fa-file-pdf"></i> PDF</button>
  ...
  `).join('') : '<tr><td colspan="8" class="empty-state">No hay tickets guardados para este técnico.</td></tr>';
}
...
function renderEvidence() {
  ...
  APP.images = mergeEvidenceListsFinal(APP.images || []);
  counter.textContent = `${APP.images.length} ${APP.images.length === 1 ? 'imagen' : 'imágenes'}`;
  grid.innerHTML = APP.images.map((rawImage, index) => {
    const image = normalizeEvidenceItem(rawImage, index) || { name: `Evidencia ${index + 1}`, dataUrl: '', previewUrl: '', sourceUrl: '' };
    const candidates = image.previewCandidates || buildEvidencePreviewCandidates(image.previewUrl || image.sourceUrl || image.dataUrl || '');
    const previewSrc = safeText(image.dataUrl || image.previewUrl || candidates[0] || image.sourceUrl);
    const driveId = extractDriveFileId(image.sourceUrl || image.previewUrl || candidates.join(' '));
    const openUrl = safeText(image.sourceUrl || (driveId ? `https://drive.google.com/file/d/${driveId}/view?usp=sharing` : image.previewUrl));
    const linkHtml = openUrl && !openUrl.startsWith('data:image') ? `<a class="evidence-drive-link" href="${escapeHtml(openUrl)}" target="_blank" rel="noopener noreferrer">Abrir imagen</a>` : '';
    return `
      <div class="evidence-item ${previewSrc ? '' : 'no-preview'}">
        ${previewSrc ? `<img src="${escapeHtml(previewSrc)}" alt="Evidencia ${index + 1}" data-index="${index}" loading="lazy">` : `<div class="evidence-placeholder">Sin vista previa${linkHtml}</div>`}
        <button type="button" class="evidence-remove" data-index="${index}"><i class="fas fa-trash"></i></button>
        <div class="evidence-caption">${escapeHtml(safeText(image.name) || `Evidencia ${index + 1}`)}${linkHtml && previewSrc ? `<small>${linkHtml}</small>` : ''}</div>
      </div>
    `;
  }).join('');
  grid.querySelectorAll('img[data-index]').forEach(img => {
    img.onerror = () => {
      const idx = Number(img.dataset.index);
      const item = APP.images[idx];
      const normalized = normalizeEvidenceItem(item, idx) || {};
      const candidates = [...new Set([...(normalized.previewCandidates || []), ...buildEvidencePreviewCandidates(normalized.sourceUrl || normalized.previewUrl || '')].filter(Boolean))];
      const current = safeText(img.getAttribute('src'));
      const next = candidates.find(url => safeText(url) && safeText(url) !== current);
      if (next) {
        img.src = next;
      ...
      const wrapper = img.closest('.evidence-item');
      const driveId = extractDriveFileId(normalized.sourceUrl || normalized.previewUrl || candidates.join(' '));
      const openUrl = safeText(normalized.sourceUrl || (driveId ? `https://drive.google.com/file/d/${driveId}/view?usp=sharing` : ''));
      if (wrapper) {
        wrapper.classList.add('no-preview');
        img.remove();
        wrapper.insertAdjacentHTML('afterbegin', `<div class="evidence-placeholder">Sin vista previa${openUrl ? `<a class="evidence-drive-link" href="${escapeHtml(openUrl)}" target="_blank" rel="noopener noreferrer">Abrir imagen</a>` : ''}</div>`);
      }
    };
  });
}
...
async function editTicketByRef(ref) {
  ...
  $('casoTienda').value = ticket.tienda || '';
  $('casoServicio').value = ticket.servicio || '';
  if ($('casoStatus')) $('casoStatus').value = normalizeStatusLabel(ticket.status) || '';
  updateStatusFieldVisibility();
  if ($('casoEscaladaArea')) $('casoEscaladaArea').value = getTicketEscaladaArea(ticket) || '';
  renderDynamicForm();
  (ticket.dynamicFields || []).forEach(field => { if ($(field.id)) $(field.id).value = field.value || ''; });
  ...
  updateStatusFieldVisibility();
  if ($('casoEscaladaArea')) $('casoEscaladaArea').value = getTicketEscaladaArea(ticket) || '';
  APP.images = mergeEvidenceListsFinal(ticket.evidencias || [], ticket.raw ? extractEvidenceFromRecord(ticket.raw) : []);
  renderEvidence();
  hydrateEvidenceDataUrls(APP.images, true).catch(() => {});
  restoreChecklistAnswers(ticket.checklists || { checklist1: [], checklist2: [] });
  openView('casosView', 'Casos Técnicos');
...
async function downloadTicketByRef(ref) {
  ...
  if (!ticket) return;
  const enriched = enrichTicketEvidenceFinal(ticket);
  enriched.evidencias = mergeEvidenceListsFinal(enriched.evidencias || [], enriched.raw ? extractEvidenceFromRecord(enriched.raw) : []);
  await hydrateEvidenceDataUrls(enriched.evidencias).catch(() => {});
  await generarPDF(enriched);
}

function getTicketMetricStatus(ticket) {
  return getTicketDisplayStatusFinal(ticket);
}
...
window.editTicketByRef = editTicketByRef;
window.downloadTicketByRef = downloadTicketByRef;
